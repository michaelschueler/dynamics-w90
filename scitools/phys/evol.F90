...
!--------------------------------------------------------------------------------------
   pure subroutine UnitaryStepFW(n,Ut,Xt,Xtdt,large_size)
      !! peforms the operation \(X(t+\Delta t) = U(\Delta t)X(t)\)
      integer,intent(in)                        :: n !! matrix dimension
      complex(dp),dimension(:,:),intent(in)     :: Ut !! time evolution operator U(\Delta t)
      complex(dp),dimension(:,:),intent(in)     :: Xt !! matrix X(t)
      complex(dp),dimension(:,:),intent(inout)  :: Xtdt !! matrix X(t+\Delta t)
...
!--------------------------------------------------------------------------------------
   pure subroutine UnitaryStepBW(n,Ut,Xt,Xtdt,large_size)
      !! peforms the operation \(X(t+\Delta t) = X(t)U^\dagger(\Delta t)\)
      integer,intent(in)                        :: n !! matrix dimension
      complex(dp),dimension(:,:),intent(in)     :: Ut !! time evolution operator U(\Delta t)
      complex(dp),dimension(:,:),intent(in)     :: Xt !! matrix X(t)
      complex(dp),dimension(:,:),intent(inout)  :: Xtdt !! matrix X(t+\Delta t)
...
!--------------------------------------------------------------------------------------
   pure subroutine UnitaryStepFBW(n,Ut,Xt,Xtdt,large_size)
      !! peforms the operation \(X(t+\Delta t) = U(\Delta t)X(t)U^\dagger(\Delta t)\)
      integer,intent(in)                        :: n !! matrix dimension
      complex(dp),dimension(:,:),intent(in)     :: Ut !! time evolution operator U(\Delta t)
      complex(dp),dimension(:,:),intent(in)     :: Xt !! matrix X(t)
      complex(dp),dimension(:,:),intent(inout)  :: Xtdt !! matrix X(t+\Delta t)
...
!--------------------------------------------------------------------------------------    
   subroutine GenU_CF2(dt,Hdt2,Udt)
      ...
      !!  \( U(t+\Delta t,t) = exp[-i \Delta t H(t+\Delta t/2)] \)
      real(dp),intent(in)       :: dt !! time step
      complex(dp),intent(in)    :: Hdt2(:,:) !! Hamiltonian at t+\Delta t/2
...
!--------------------------------------------------------------------------------------
   subroutine GenU_CF4(dt,H1,H2,Udt,large_size)
      ...
      !! $$ U(t+\Delta t,t) = \exp[-i \Delta t (a_1 H_1 + a_2 H_2] \exp[-i \Delta t (a_2 H_1 + a_1 H_2)] , $$ where
      !! \(H_1 = H(t + c_1 \Delta t)\), \(H_2 = H(t + c_2 \Delta t)\) and
      ...
      real(dp),intent(in)         :: dt !! time step
      complex(dp),intent(in)      :: H1(:,:) !! Hamiltonian at t + c1 * \Delta t
      complex(dp),intent(in)      :: H2(:,:) !! Hamiltonian at t + c2 * \Delta t
      complex(dp),intent(inout)   :: Udt(:,:) !! time evolution operator
      logical,intent(in),optional :: large_size !! if .true., ZGEMM will be called for matrix-matrix multiplication
      logical :: use_zgemm
      integer :: n
