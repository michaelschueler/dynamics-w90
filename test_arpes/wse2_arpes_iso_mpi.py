...
def PrepKpoints(kxr,kyr,nk1,nk2):
	...
	return file_kpts
#---------------------------------------------------------------------
def run_calc(mpicmd,wphot,mu,Eshift,Epe,alpha,phi=0.0):
	...
	exe = os.environ['HOME'] + '/Programs/dynamics-w90/exe/arpes_mpi.x'
	...
	os.system(mpicmd + ' ' + exe + ' ' + file_inp + ' ' + outpref)
...
def main(mpicmd):
	...
	run_calc(mpicmd,wphot,mu,Eshift,Epe,alpha,phi=phi)
#---------------------------------------------------------------------
