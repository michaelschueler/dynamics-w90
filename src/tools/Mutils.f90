...
contains
  ...
  subroutine load_griddata_real(filename, grid, d)
    character(len=*), intent(in) :: filename
    ! The array 'd' will be automatically allocated with the correct dimensions
    real(dp), allocatable, intent(out) :: grid(:), d(:, :)
    ...
    if(.not.allocated(grid)) allocate(grid(nrow))
    if(.not.allocated(d)) allocate(d(nrow, ncol))
  ...
  subroutine load_griddata_complex(filename, grid, d)
    character(len=*), intent(in) :: filename
    ! The array 'd' will be automatically allocated with the correct dimensions
    ...
    if(.not.allocated(grid)) allocate(grid(nrow))
    if(.not.allocated(d)) allocate(d(nrow, (ncol-1)/2))
    allocate(colvec(ncol-1))
    do i = 1, nrow
       read(s, *) grid(i), colvec(1:ncol-1)
       d(i,1:(ncol-1)/2)=cmplx(colvec(1:(ncol-1)/2),colvec((ncol-1)/2+1:ncol-1),dp)
    end do
    close(s)
    deallocate(colvec)
