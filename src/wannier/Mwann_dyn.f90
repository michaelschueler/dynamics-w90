...
!--------------------------------------------------------------------------------------
   subroutine Wann_Rhok_timestep_velo(w90,Nk,kpts,tstp,dt,field,Rhok)
      ...
      !$OMP PARALLEL DO PRIVATE(Rho_old,H1,H2,vk,Udt)
      do ik=1,Nk
         Rho_old = Rhok(:,:,ik)
...
!--------------------------------------------------------------------------------------
   subroutine Wann_Rhok_timestep_velo_calc(nbnd,Nk,Hk,vk,tstp,dt,field,Rhok)
      ...
      !$OMP PARALLEL DO PRIVATE(Rho_old,H1,H2,Udt)
      do ik=1,Nk
         Rho_old = Rhok(:,:,ik)
...
!--------------------------------------------------------------------------------------
   subroutine Wann_Rhok_timestep_dip_field(w90,Nk,kpts,tstp,dt,field,Rhok,Peierls_only)
      ...
      logical :: peierls_
      integer :: ik
      real(dp) :: EF_1(2),AF_1(2),EF_2(2),AF_2(2)
      complex(dp),dimension(w90%num_wann,w90%num_wann) :: H1,H2,Rho_old,Udt
...
!--------------------------------------------------------------------------------------
   subroutine Wann_Rhok_timestep_dip_free(w90,Nk,kpts,tstp,dt,AF,Rhok)
      ...
      !$OMP PARALLEL DO PRIVATE(Rho_old,H1,Udt)
      do ik=1,Nk
         Rho_old = Rhok(:,:,ik)
...
!--------------------------------------------------------------------------------------
   function Wann_Current_para_velo(w90,Nk,kpts,Rhok) result(current)
      type(wann90_tb_t),intent(in) :: w90
      integer,intent(in)           :: Nk
      real(dp),intent(in)          :: kpts(:,:)
      ...
      real(dp)                     :: current(3)
      integer :: ik
      real(dp) :: Jk(3),Jpara(3)
      complex(dp),dimension(w90%num_wann,w90%num_wann,3) :: vk

      Jpara = 0.0_dp

      !$OMP PARALLEL PRIVATE(vk,Jk)
      !$OMP DO REDUCTION(+:Jpara) 
      do ik=1,Nk
         vk = w90%get_velocity([kpts(ik,1),kpts(ik,2),kpts(ik,3)])
...
!--------------------------------------------------------------------------------------
   function Wann_Current_para_velo_calc(nbnd,Nk,vk,Rhok) result(current)
      integer,intent(in)           :: nbnd,Nk
      complex(dp),intent(in)       :: vk(:,:,:,:)
      complex(dp),intent(in)       :: Rhok(:,:,:)
      real(dp)                     :: current(3)
...
!--------------------------------------------------------------------------------------
   function Wann_Current_dia_velo(Nk,Avec,Rhok) result(current)
      ...
      npart = 0.0_dp
      !$OMP PARALLEL DO REDUCTION(+:npart) 
      do ik=1,Nk
...
!--------------------------------------------------------------------------------------
   function Wann_Current_Intra_velo(w90,Nk,kpts,Rhok) result(current)
      use Mlinalg,only: Eye
      type(wann90_tb_t),intent(in) :: w90
      integer,intent(in)           :: Nk
      real(dp),intent(in)          :: kpts(:,:)
      ...
      do ik=1,Nk
         ...
         Jk = 0.0_dp
         do i=1,w90%num_wann
            Jk(1) = Jk(1) + qc * dble(vk(i,i,1)*Rhok(i,i,ik))
            Jk(2) = Jk(2) + qc * dble(vk(i,i,2)*Rhok(i,i,ik))
            Jk(3) = Jk(3) + qc * dble(vk(i,i,3)*Rhok(i,i,ik))
         end do
         Jcurr = Jcurr + Jk
      end do
      !$OMP END DO
...
!--------------------------------------------------------------------------------------
   function Wann_Current_Intra_velo_calc(nbnd,Nk,vk,Rhok) result(current)
      integer,intent(in)           :: nbnd,Nk
      complex(dp),intent(in)       :: vk(:,:,:,:)
      complex(dp),intent(in)       :: Rhok(:,:,:)
      real(dp)                     :: current(3)
      integer :: ik,i
      real(dp) :: Jk(2),Jcurr(2)
...
!--------------------------------------------------------------------------------------
   function Wann_Pol_velo(w90,Nk,kpts,Rhok,band_basis) result(dipole)
      ...
      integer :: ik
      real(dp) :: Dip(2),Dipk(2)
      complex(dp),dimension(w90%num_wann,w90%num_wann,3) :: Dk

      if(present(band_basis)) band_basis_ = band_basis

      Dip = 0.0_dp
      !$OMP PARALLEL DO REDUCTION(+:dip) PRIVATE(Dk,Dipk)
      do ik=1,Nk
...
!--------------------------------------------------------------------------------------
   function Wann_Pol_dip(w90,Nk,kpts,Avec,Rhok,rot_mat) result(dipole)
      ...
      if(present(rot_mat)) then
         !$OMP PARALLEL DO REDUCTION(+:dip) PRIVATE(Dk,Dipk)
         do ik=1,Nk
            Dk = w90%get_dipole([kpts(ik,1)-Ared(1),kpts(ik,2)-Ared(2),kpts(ik,3)-Ared(3)],band_basis=.false.)
            do idir=1,3
...
!--------------------------------------------------------------------------------------
   function Wann_Pol_dip_calc(nbnd,Nk,Dk,Rhok) result(dipole)
      ...
      real(dp)                     :: dipole(3)
      integer :: ik
      real(dp) :: Dipk(3),dip(3)
 
      Dip = 0.0_dp

      !$OMP PARALLEL DO REDUCTION(+:dip) PRIVATE(Dipk)
      do ik=1,Nk
...
!--------------------------------------------------------------------------------------
   subroutine Wann_Current_dip_kpt(w90,Nk,kpts,Avec,EF,Rhok,Jk)
      ...
      !$OMP PARALLEL DO PRIVATE(DRhok_dt,grad_Hk,Dk,kAred)
      do ik=1,Nk
         kAred = kpts(ik,:) - Ared
...
!--------------------------------------------------------------------------------------
   function Wann_Current_dip(w90,Nk,kpts,Avec,EF,Rhok,dipole_current,rot_mat) result(current)
      type(wann90_tb_t),intent(in) :: w90
      integer,intent(in)           :: Nk
      real(dp),intent(in)          :: kpts(:,:)
      real(dp),intent(in)          :: Avec(3),EF(3)
      ...
      logical :: dip_curr
      integer :: ik,idir
      real(dp) :: Ared(3),kAred(3),Jk(3),Jcurr(3)
      complex(dp),dimension(w90%num_wann,w90%num_wann)   :: DRhok_dt
      complex(dp),dimension(w90%num_wann,w90%num_wann,3) :: grad_Hk,Dk
      ...
      if(present(rot_mat)) then
         !$OMP PARALLEL PRIVATE(DRhok_dt,grad_Hk,Dk,Jk,kAred)
         !$OMP DO REDUCTION(+:Jcurr) 
         do ik=1,Nk
            kAred = kpts(ik,:) - Ared
            grad_Hk = w90%get_gradk_ham(kAred) 
...
!--------------------------------------------------------------------------------------
   function Wann_Current_dip_calc(nbnd,Nk,Hk,grad_Hk,Dk,Rhok,dipole_current) result(current)
      integer,intent(in)           :: nbnd,Nk
      complex(dp),intent(in)       :: Hk(:,:,:)
      ...
      !$OMP PARALLEL PRIVATE(DRhok_dt,Jk)
      !$OMP DO REDUCTION(+:Jcurr) 
      do ik=1,Nk
...
!--------------------------------------------------------------------------------------
   function Wann_TotalEn_velo(w90,Nk,kpts,Avec,Rhok) result(Etot)
      ...
      Etot = 0.0_dp
      !$OMP PARALLEL DO REDUCTION(+:Etot) PRIVATE(Hk,vk,Ek)
      do ik=1,Nk
...
!--------------------------------------------------------------------------------------
   function Wann_TotalEn_velo_calc(nbnd,Nk,Hk,vk,Avec,Rhok) result(Etot)
      ...
      integer :: ik
      real(dp) :: Ek
      complex(dp) :: Hk_1(nbnd,nbnd)

      Etot = 0.0_dp
      !$OMP PARALLEL DO REDUCTION(+:Etot) PRIVATE(Ek)
      do ik=1,Nk
         Hk_1 = Hk(:,:,ik) - qc*(Avec(1)*vk(:,:,ik,1) + Avec(2)*vk(:,:,ik,2)) + Avec(3)*vk(:,:,ik,3))
...
!--------------------------------------------------------------------------------------
   function Wann_KineticEn(w90,Nk,kpts,Rhok,band_basis) result(Ekin)
      ...
      if(bands_) then
         !$OMP PARALLEL DO REDUCTION(+:Ekin) PRIVATE(Hk,Ek)
         do ik=1,Nk
            Hk = w90%get_ham_diag([kpts(ik,1),kpts(ik,2),kpts(ik,3)]) 
            Ek = DTRAB(w90%num_wann,Hk,Rhok(:,:,ik))
            Ekin = Ekin + Ek/Nk
...
!--------------------------------------------------------------------------------------
   function Wann_KineticEn_calc(nbnd,Nk,Hk,Rhok) result(Ekin)
      ...
      !$OMP PARALLEL DO REDUCTION(+:Ekin) PRIVATE(Ek)
      do ik=1,Nk
         Ek = DTRAB(nbnd,Hk(:,:,ik),Rhok(:,:,ik))
         Ekin = Ekin + Ek/Nk
      end do
...
!--------------------------------------------------------------------------------------
   function Wann_TotalEn_dip(w90,Nk,kpts,Avec,Efield,Rhok) result(Etot)
      ...
      Etot = 0.0_dp
      !$OMP PARALLEL DO REDUCTION(+:Etot) PRIVATE(Hk,Dk,Ek,kAred)
      do ik=1,Nk
         kAred = kpts(ik,:) - Ared
