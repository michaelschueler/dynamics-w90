...
contains
!--------------------------------------------------------------------------------------
   elemental function SlaterWF_radial(r,Z,n,l) result(Rrad)
      ...
      select case ( orb )
      case('1s')
         Rrad = 2d0 * (Z**1.5d0) * Exp(-rho/2d0)
      case('2s')
         Rrad = 1d0/(2d0*sqrt(2d0)) * ( 2d0 - rho ) * (Z**1.5d0) * Exp(-rho/2d0)
      case('2p')
         Rrad = 1d0/(2d0*sqrt(6d0)) * rho * (Z**1.5d0) * Exp(-rho/2d0)
      case('3s')
         Rrad = 1d0/(9d0*sqrt(3d0)) * ( 6d0 - 6d0*rho + rho**2) * (Z**1.5d0) * Exp(-rho/2d0)
      case('3p')
         Rrad = 1d0/(9d0*sqrt(6d0)) * rho * ( 4d0 - rho ) * (Z**1.5d0) * Exp(-rho/2d0)
      case('3d')
         Rrad = 1d0/(9d0*sqrt(30d0)) * (rho**2) * (Z**1.5d0) * Exp(-rho/2d0)
      case('4s')
         Rrad = 1d0/(96d0) * (24d0 - 36d0*rho + 12d0*(rho**2) - rho**3) * (Z**1.5d0) * Exp(-rho/2d0)
      case('4p')
         Rrad = 1d0/(32d0*sqrt(15d0)) * rho* (20d0 - 10d0*rho + rho**2 ) * (Z**1.5d0) * Exp(-rho/2d0)
      case('4d')
         Rrad = 1d0/(96d0*sqrt(5d0)) * (rho**2) * (6d0 - rho) * (Z**1.5d0) * Exp(-rho/2d0)
      case('4f')
         Rrad = 1d0/(96d0*sqrt(35d0)) * rho**3 * Exp(-rho/2d0)
      case('5s')
         Rrad = 1d0/(300d0*sqrt(5d0)) * (120d0 - 240d0*rho + 120d0*(rho**2) &
            - 20d0*(rho**3)+rho**4) * (Z**1.5d0) * Exp(-rho/2d0)
      case('5p')
         Rrad = 1d0/(150d0*sqrt(30d0)) * rho * (120d0 - 90d0*rho + 18d0*(rho**2) - rho**3) * (Z**1.5d0) * Exp(-rho/2d0)
      case('5d')
         Rrad = 1d0/(150d0*sqrt(70d0)) * (rho**2) * (42d0 - 14d0*rho + rho**2) * (Z**1.5d0) * Exp(-rho/2d0)
      case('6s')
         Rrad = 1d0/(2160d0*sqrt(6d0)) * (720d0 - 1800d0*rho + 1200d0*(rho**2) - 300d0*(rho**3) &
            + 30d0*(rho**4) - rho**5) * (Z**1.5d0) * Exp(-rho/2d0)
      case('6p')
         Rrad = 1d0/(432d0*sqrt(210d0)) * rho * (840d0 - 840d0*rho + 252d0*(rho**2) &
            - 28d0*(rho**3) + (rho**4)) * (Z**1.5d0) * Exp(-rho/2d0)
      case('6d')
         Rrad = 1d0/(864d0*sqrt(105d0)) * (rho**2) * (336d0 - 168d0*rho + 24d0*(rho**2) &
            - (rho**3) ) * (Z**1.5d0) * Exp(-rho/2d0)
      case default
         Rrad = 0d0
      end select
...
!--------------------------------------------------------------------------------------
   elemental function SlaterWF_radial_deriv(r,Z,n,l) result(Rrad)
      character(len=1),parameter ::  angular(0:3)=['s','p','d','f']
      real(dp),intent(in)         :: r
      real(dp),intent(in)         :: Z
      integer,intent(in)          :: n,l
      real(dp)                    :: Rrad
      real(dp) :: rho,Prho,dPdrho
      character(len=2) :: orb
      ...
      Rrad = 2d0 * Z / dble(n) * (Z**1.5d0) * Exp(-rho/2d0) * (dPdrho - 0.5d0*Prho) 
